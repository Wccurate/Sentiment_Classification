...
from models.common.common_bert_wrapper import BertWithClassifier
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument("--wandb-project", type=str, default="intent-recognition-bert-head",
...
def main():
	...
	if use_wandb and accelerator.is_main_process:
		...
		if run_name is None:
			run_name = f"{args.dataset}_bert_head_lr{args.lr}"
		
		# Parse tags
		tags = [t.strip() for t in args.wandb_tags.split(",") if t.strip()]
		tags.extend([args.dataset, "bert", "classifier-head"])
		...
		wandb.init(
			...
			config={
				...
				"head_learning_rate": args.lr * 10.0,
	...
	# Build model
	model = BertWithClassifier(model_name=args.model_path, num_labels=num_labels, dropout=0.1, freeze_encoder=True)
	...
	# Optimizer and scheduler with differential learning rates
	# Classification head gets 10x higher learning rate than encoder (if trainable)
	head_params = [p for n, p in model.named_parameters() 
	               if p.requires_grad and ("classifier" in n or "score" in n)]
	encoder_params = [p for n, p in model.named_parameters() 
	                  if p.requires_grad and not ("classifier" in n or "score" in n)]
	
	optimizer_grouped_parameters = []
	if head_params:
		optimizer_grouped_parameters.append({
			"params": head_params,
			"lr": args.lr * 10.0,  # 10x learning rate for classification head
			"weight_decay": args.weight_decay,
		})
	if encoder_params:
		optimizer_grouped_parameters.append({
	...
	accelerator.print(f"  Head learning rate: {args.lr * 10.0:.2e}")
	accelerator.print(f"  Encoder learning rate: {args.lr:.2e} (if trainable)")
		...
		if best_dir.exists():
			try:
				artifact = wandb.Artifact(
					name=f"{args.dataset}_bert_head_best",
					type="model",
					description=f"Best BERT classifier head for {args.dataset} dataset",
					metadata={
