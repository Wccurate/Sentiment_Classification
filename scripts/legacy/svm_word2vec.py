#!/usr/bin/env python3
"""
SVM with Word2Vec (Average Pooling) for binary sentiment classification on Project dataset.
Features:
...
# ========== Word2Vec Training and Sentence Encoding ==========
...
def train_word2vec(
    ...
    print(f"  Unique words (before min_count filter): {len(unique_words)}")
    ...
    print(f"  Vocabulary size (after min_count={min_count}): {vocab_size}")
...
def sentence_to_vector(
    ...
    """
    Convert sentence to vector using average pooling of word embeddings.
...
def texts_to_vectors(
    ...
    """
    Convert list of texts to matrix of sentence vectors.
    ...
    Returns:
        Matrix of sentence vectors (n_samples, vector_size)
    """
    vectors = []
    
    iterator = tqdm(texts, desc="Encoding sentences") if show_progress else texts
    
    for text in iterator:
        vector = sentence_to_vector(text, w2v_model, vector_size)
...
def cross_validate_svm(
    ...
    Args:
        X: Feature matrix (Word2Vec sentence vectors)
        y: Labels
...
def final_train_test_evaluation(
    ...
    gamma: str = 'scale',
    random_state: int = 42,
    output_dir: str = "outputs/svm_word2vec_results"
) -> dict:
    """
    ...
    Args:
        ...
        vector_size: Word2Vec embedding dimension
        window: Word2Vec context window
        min_count: Word2Vec minimum word frequency
    ...
    # Convert texts to sentence vectors using average pooling
    print(f"\n{'='*80}")
    print("Encoding Sentences with Average Pooling")
    print(f"{'='*80}")
    
    print("\nEncoding training set...")
    X_train_vectors = texts_to_vectors(
    ...
    print("\nEncoding test set...")
    X_test_vectors = texts_to_vectors(
    ...
    print(f"\nSentence vector shape: {X_train_vectors.shape}")
    ...
    for text in train_df[text_column]:
        tokens = tokenize_text(text)
        train_total_tokens += len(tokens)
        train_oov_count += sum(1 for token in tokens if token not in w2v_model.wv)
    ...
    for text in test_df[text_column]:
        ...
        test_oov_count += sum(1 for token in tokens if token not in w2v_model.wv)
    
    train_oov_rate = train_oov_count / train_total_tokens if train_total_tokens > 0 else 0
    test_oov_rate = test_oov_count / test_total_tokens if test_total_tokens > 0 else 0
    
    print(f"\nOut-of-Vocabulary (OOV) Statistics:")
    print(f"  Train OOV rate: {train_oov_rate:.2%} ({train_oov_count}/{train_total_tokens})")
    print(f"  Test OOV rate:  {test_oov_rate:.2%} ({test_oov_count}/{test_total_tokens})")
    ...
    X_train_scaled = scaler.transform(X_train_vectors)
    X_train_pca = pca.transform(X_train_scaled)
    ...
    w2v_model.save(str(output_path / "word2vec_model.bin"))
    ...
    evaluation_results = {
        "model_type": "SVM with RBF kernel + Word2Vec Average Pooling",
        ...
        "vocabulary_stats": {
            ...
            "train_oov_rate": float(train_oov_rate),
            "test_oov_rate": float(test_oov_rate)
        },
        "train_metrics": {
    ...
    with open(output_path / "results_summary.txt", "w", encoding="utf-8") as f:
        f.write("="*80 + "\n")
        f.write("SVM + Word2Vec (Average Pooling) Classification Results\n")
        f.write("="*80 + "\n\n")
        ...
        f.write(f"Embedding: Word2Vec with Average Pooling\n")
        ...
        f.write(f"  Train OOV rate: {train_oov_rate:.2%}\n")
        f.write(f"  Test OOV rate: {test_oov_rate:.2%}\n\n")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="SVM + Word2Vec (Average Pooling) for binary sentiment classification"
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Word2Vec minimum word frequency"
    )
    parser.add_argument(
        "--w2v-epochs",
        type=int,
...
def main():
    ...
    print("="*80)
    print("SVM + Word2Vec (Average Pooling) Binary Sentiment Classification")
    print("="*80)
    print(f"\nConfiguration:")
    ...
    print(f"  Word2Vec vector size: {args.vector_size}")
    print(f"  Word2Vec window: {args.window}")
    print(f"  Word2Vec min count: {args.min_count}")
    ...
    if not args.skip_cv:
        ...
        # Train Word2Vec on entire dataset for CV
        print("\nTraining Word2Vec on full dataset for cross-validation...")
        w2v_model_cv = train_word2vec(
        ...
        print("\nEncoding all texts with average pooling...")
        X_cv = texts_to_vectors(
        ...
        print(f"\nSentence vectors for CV: {X_cv.shape}")
